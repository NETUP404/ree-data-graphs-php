...
// Función para cargar los scripts necesarios para el gráfico
function ree_enqueue_chartjs() {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const tablasComparativas = document.querySelectorAll('.ree-table-comparativa tbody');
        const tablasPreciosDia = document.querySelectorAll('.ree-table-precio-dia tbody');

        tablasComparativas.forEach(tabla => {
            function resaltarCeldasComparativa() {
                const filas = tabla.querySelectorAll('tr');
                let minValor = Infinity;
                let maxValor = -Infinity;
                let minCelda, maxCelda;

                filas.forEach(fila => {
                    const celdas = fila.querySelectorAll('td');
                    const valorMax = parseFloat(celdas[0].textContent.replace('€', '').trim());
                    const valorMin = parseFloat(celdas[2].textContent.replace('€', '').trim());

                    if (valorMax > maxValor) {
                        maxValor = valorMax;
                        maxCelda = celdas[0];
                    }
                    if (valorMin < minValor) {
                        minValor = valorMin;
                        minCelda = celdas[2];
                    }
                });
            ...
            resaltarCeldasComparativa();
        });

        tablasPreciosDia.forEach(tabla => {
            function resaltarCeldasPreciosDia() {
                const filas = tabla.querySelectorAll('tr');
                let minValor = Infinity;
                let maxValor = -Infinity;
                let minCelda, maxCelda;
                ...
                filas.forEach(fila => {
                    const celdas = fila.querySelectorAll('td');
                    const valor = parseFloat(celdas[1].textContent.replace('€', '').trim());
            ...
            resaltarCeldasPreciosDia();
        });
    });
    ";
    wp_add_inline_script('chartjs', $custom_js);
}

add_action('wp_enqueue_scripts', 'ree_enqueue_chartjs');

// Función para cargar los estilos necesarios para los gráficos
function ree_enqueue_styles() {
    if (is_singular() && (has_shortcode(get_post()->post_content, 'ree_grafico_dia') || 
        has_shortcode(get_post()->post_content, 'ree_grafico_semana') || 
        has_shortcode(get_post()->post_content, 'ree_grafico_mes') || 
        has_shortcode(get_post()->post_content, 'ree_grafico_anio') || 
        has_shortcode(get_post()->post_content, 'ree_grafico_dia_siguiente') || 
        has_shortcode(get_post()->post_content, 'ree_tabla_precio_dia') || 
        has_shortcode(get_post()->post_content, 'ree_tabla_comparativa'))) {
        
        // Estilos CSS para los gráficos y las tablas
        $css = "
            canvas {
                max-width: 100%;
            ...
            .ree-table, .ree-table th, .ree-table td {
                border: 1px solid #ddd;
            }

            .ree-table th, .ree-table td {
                padding: 8px;
                text-align: center;
            }
        ...
        ";
        wp_add_inline_style('ree-styles', $css);
    }
}

add_action('wp_enqueue_scripts', 'ree_enqueue_styles');
...
function ree_obtener_datos_api($start_date, $end_date) {
    ...
    $url = sprintf(
        "https://apidatos.ree.es/es/datos/mercados/precios-mercados-tiempo-real?start_date=%s&end_date=%s&time_trunc=hour",
...
// Función para procesar los datos y mostrar el gráfico y la tabla
function ree_procesar_datos($start_date, $end_date, $rango = 'horas') {
    $data = ree_obtener_datos_api($start_date, $end_date);
    $json_data = json_decode($data, true);

    if (empty($json_data) || !isset($json_data['included'][0]['attributes']['values'])) {
        return null;
    }

    $values = array_map(function($item) {
        return $item['value'] / 1000; // Convertir €/MWh a €/kWh
    ...
    $labels = array_map(function($item) use ($rango, $dias_semana) {
        ...
        if ($rango == 'dias') {
            return $dias_semana[$datetime->format('w')] . ' ' . $datetime->format('d'); // Formato de día de la semana y día del mes
        } else {
            return $datetime->format('H') . 'h'; // Solo la hora
        }
    }, $json_data['included'][0]['attributes']['values']);
...
function ree_grafico_dia_siguiente() {
    ...
    $data = ree_procesar_datos($start_date, $end_date, 'horas');
    
    if (!$data) {
        return '<div style="border: 1px solid #ddd; padding: 10px; background-color: #f8f8f8; text-align: center;">Los precios de la luz de mañana serán mostrados a partir de las 20:00.</div>';
    }
...
function ree_tabla_comparativa_dia_siguiente() {
    ...
    $max_price = max($prices);
    $min_price = min($prices);
    $current_hour = (int)date('H', strtotime('tomorrow'));
    $current_price = $prices[$current_hour];
...
// Registrar los nuevos shortcodes
